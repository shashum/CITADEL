...
FILE_PATHS = {
    ...
    "attack_surfaces": "attack_surface_en.json",
    "attack_vectors": "attack_vector_en.json",
    "campaigns": "campaign-merge.json",
    "complexity": "complexity.json",
    "industry_desc": "critical-infra-description.json"
}
...
def load_all_data(paths):
    ...
    try:
        ...
        for key, path in paths.items():
            if os.path.exists(path):
                if key == "prompt_template":
                    ...
                    with open(path, 'r', encoding='utf-8') as f: data[key.split('.')[0]] = json.load(f)
            else: raise FileNotFoundError(f"필수 파일 '{path}'를 찾을 수 없습니다.")

        # 바이너리, 모델 파일 로드
        data['rag_index'] = faiss.read_index(paths["faiss_index"])
        with open(paths["capec_data"], "rb") as f: data['capec_data'] = pickle.load(f)
        data['rag_model'] = SentenceTransformer(paths["embedding_model"])
        ...
        data['campaigns_by_name'] = data['campaign-merge']['campaigns'] # campaign-merge.json 구조에 맞춤
...
def search_capec(query, top_k=1, rag_model=None, rag_index=None, rag_data=None):
    """CAPEC 벡터DB에서 관련 정보를 검색하는 함수"""
    if not all([rag_model, rag_index, rag_data]): return "No CAPEC context available."
    query_vector = rag_model.encode([query])
    _, indices = rag_index.search(np.array(query_vector).astype('float32'), top_k)
    return rag_data['documents'][indices[0][0]]

# --- 3. TTP 공격 체인 구성 함수 (최종 수정 버전) ---

def build_attack_chain(config, cti_data):
    """[최종 수정됨] 설정값에 따라 동적으로 TTP 공격 체인을 구성하는 함수"""
    ...
    # [핵심 수정] CTI 데이터의 'tactics' 필드를 사용하여 기술을 전술별로 그룹화
...
# --- 4. 최종 프롬프트 조합 함수 (개선 버전) ---

def assemble_final_prompt(config, data, attack_chain):
    """모든 정보를 최종 마스터 프롬프트 템플릿에 삽입하는 함수"""
    ...
    if config['GENERATION_MODE'] == 'Actor-Emulation':
        ...
        replacements["{{CAMPAIGN_NAME}}"] = config['CAMPAIGN_NAME']
        replacements["{{GROUP_ID_OR_CAMPAIGN_ID}}"] = campaign_info.get('external_references', [{}])[0].get('external_id', 'N/A')
        replacements["{{CAMPAIGN_DESCRIPTION}}"] = campaign_info.get('description', 'N/A')
        replacements["{{CAMPAIGN_GOAL}}"] = campaign_info.get('primary_motivation', 'Not specified')
        # 불필요한 태그 제거
        ...
        replacements["{{TACTIC_ID}}"] = next((t['id'] for t in attack_chain if t['technique_id'] == config['FOCUS_TECHNIQUE_ID']), 'N/A')
        replacements["{{TACTIC_NAME}}"] = tech_info.get('tactics', ['N/A'])[0]
        replacements["{{TECHNIQUE_ID}}"] = config['FOCUS_TECHNIQUE_ID']
        replacements["{{TECHNIQUE_NAME}}"] = tech_info.get('name', 'N/A')
        # 불필요한 태그 제거
        prompt = re.sub(r'', '', prompt, flags=re.DOTALL)
    ...
    for stage in attack_chain:
        ...
        capec_context = search_capec(query, 1, data['rag_model'], data['rag_index'], data['rag_data'])
        attack_chain_str += f"""
...
def generate_scenario(prompt, model_name):
    ...
    outputs = model.generate(
        **inputs,
        max_new_tokens=4096,
        do_sample=True, temperature=0., top_p=0.9,
    ...
    if scenario_match:
        ...
        final_scenario = re.split(r'\[/INST\]|\[/s\]', full_output, maxsplit=1)[-1].strip()
